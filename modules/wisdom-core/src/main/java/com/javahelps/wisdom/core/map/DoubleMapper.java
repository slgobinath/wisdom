...
@WisdomExtension("double")
public class DoubleMapper extends Mapper {

    private final String currentName;

    public DoubleMapper(String attrName, Map<String, ?> properties) {
        super(attrName, properties);
        this.currentName = Commons.getProperty(properties, ATTR, 0);
        if (this.currentName == null) {
            throw new WisdomAppValidationException("Required property %s for Double mapper not found", ATTR);
        }
    }
    ...
    public Event map(Event event) {
        ...
        if (value instanceof Number) {
            event.set(attrName, ((Number) value).doubleValue());
        } else if (value instanceof WisdomArray) {
            event.set(attrName, ((WisdomArray) value).toDoubleArray());
        }
        return event;
    }
}
