...
class EveryPatternOperator extends PatternOperator {

    private final PatternOperator operator;

    EveryPatternOperator(PatternOperator operator) {
        this.operator = operator;
    }

    @Override
    public Pattern build(WisdomApp app, Query query) {
        Pattern pattern = Pattern.every(this.operator.build(app, query));
        return pattern;
    }
}
